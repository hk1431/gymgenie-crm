...
import { LayoutDashboard, Users, CreditCard, ClipboardCheck, MessageSquareHeart, BarChart3, Settings, LogOut, Menu, X, Wallet } from "lucide-react";
...
import { buildTasks } from "./aaj-ka-kaam";
            ...
            {nav.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.to === "/dashboard" && taskCount > 0 && (
                    <span className="ml-auto rounded-full bg-primary px-1.5 py-0.5 text-[10px] font-bold text-primary-foreground">{taskCount}</span>
                  )}
                  {active && item.to !== "/dashboard" && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-primary" />}
                </Link>
              );
            })}
          </nav>
