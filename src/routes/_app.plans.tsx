...
import { Bell, CreditCard, Plus, Printer, Receipt } from "lucide-react";
...
function PlansPage() {
  ...
  const [gstByReceipt, setGstByReceipt] = useState<Record<string, number>>({});
  const [receiptOpen, setReceiptOpen] = useState<Payment | null>(null);
  const [f, setF] = useState({ memberId: "", amount: "", mode: "UPI" as Payment["mode"], date: new Date().toISOString().slice(0,10), gst: "0" });
  ...
  const record = (e: React.FormEvent) => {
    ...
    setPays([p, ...pays]);
    setGstByReceipt(prev => ({ ...prev, [p.receiptNo]: Number(f.gst) }));
    toast.success(`Payment recorded — Receipt ${p.receiptNo}`);
    setF({ memberId: "", amount: "", mode: "UPI", date: new Date().toISOString().slice(0,10), gst: "0" });
  };
  ...
  return (
    <>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent">
                    <TableHead>Receipt</TableHead><TableHead>Member</TableHead><TableHead>Amount</TableHead><TableHead>Mode</TableHead><TableHead>Date</TableHead><TableHead className="text-right">Action</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {pays.map(p => (
                    <TableRow key={p.id}>
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" size="sm" className="gap-1.5" onClick={() => setReceiptOpen(p)}>
        ...
        <div className="space-y-6">
          <form onSubmit={record} className="rounded-xl border border-border bg-card p-5">
            ...
            <div className="space-y-3">
              ...
              <div><Label>GST</Label>
                <Select value={f.gst} onValueChange={(v)=>setF({...f, gst:v})}>
                  <SelectTrigger className="mt-1.5"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {["0","5","12","18"].map(g => <SelectItem key={g} value={g}>{g}%</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
      ...
      <ReceiptDialog
        open={!!receiptOpen}
        onOpenChange={(v) => !v && setReceiptOpen(null)}
        payment={receiptOpen}
        gstPct={receiptOpen ? (gstByReceipt[receiptOpen.receiptNo] ?? 0) : 0}
      />
    </>
  );
}
