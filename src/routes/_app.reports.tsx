...
import { revenueByMonth, memberGrowth, attendance30d, planDistribution, expenseVsRevenue } from "@/lib/mock-data";
import {
    ...
    <>
      ...
      <div className="mt-6">
        <Panel title="Revenue vs Expenses">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={expenseVsRevenue} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#ffffff10" />
              <XAxis dataKey="month" stroke="#ffffff60" fontSize={12} />
              <YAxis stroke="#ffffff60" fontSize={12} tickFormatter={(v) => `₹${(v / 1000).toFixed(0)}K`} />
              <Tooltip contentStyle={tip} formatter={(v: number, n) => [formatINR(v), n === "revenue" ? "Revenue" : "Expenses"]} />
              <Legend wrapperStyle={{ fontSize: 12, color: "#ffffff90" }} />
              <Bar dataKey="revenue" fill="#FF6B00" radius={[6, 6, 0, 0]} />
              <Bar dataKey="expense" fill="#ffffff40" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </Panel>
      </div>
